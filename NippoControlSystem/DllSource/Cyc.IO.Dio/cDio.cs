...
namespace Cyc.IO
{
    public class cDio
    {
        ...
        Cyc.IO.Log.LogLevel DIO_LogLevel = Log.LogLevel.LOG_INFO;
                    ...
                    if (regex.IsMatch(dvItem.DevName))
                    {
                        ...
                        if (Prefix == "R") //PLCリレー(Rxxxx)のみを対象
                        {
                            if (DeviceNo.Length >= 3)
                            {
                                ...
                                Cyc.IO.Log.WriteLine(DIO_LogLevel, "Cyc.IO.cDio:Init", string.Format("{1}:{2}:{3}<-{0}", dvItem.DevTitle, dvItem.DevName, (dvItem.Prefix).ToString(), dvItem.DevNo.ToString()));
                            }
                        }
                    }
                    else
